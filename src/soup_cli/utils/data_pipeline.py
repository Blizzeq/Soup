...
# v7 (#1127): the key now covers every ``DataConfig`` field that decides which rows
# land in the cache (``val_split``, ``replay*``, the streaming knobs, the media
# directories) plus the top-level ``task``. A v6 cache recorded none of them, so it
# is rejected.
_PREPROCESS_TOKENIZE_SCHEMA = "v7"

# Every ``DataConfig`` field is in exactly one of these two tables (#1127).
# ``tests/test_issue1127_preprocess_cache_key_fields.py`` walks the schema and
# fails on a field that is in neither, so a new field has to be classified before
...
def preprocess_dataset_key_input(data_cfg: Any) -> str:
    ...
    Folds in every field ``PREPROCESS_KEY_FIELDS`` routes to ``dataset_path``: a
    list ``data.train`` under a different ``data.interleave`` mixture, and since
    #1127 the split, replay and streaming settings and the media directories, all
    of which change which rows are cached. ``soup data preprocess`` and the
    ``pre_tokenized`` training gate both call this, so they cannot hash the row
    set differently: the gate once passed the raw list and
    ``make_preprocess_cache_key`` raised.

    ``getattr`` with a ``None`` default, per #761/#1101: several suites hand this a
    duck-typed stand-in carrying only ``train`` and ``interleave``.
    """
    return json.dumps(
...
def make_preprocess_cache_key(
    *,
    ...
    ``task`` is the top-level ``task`` (#1127): ``pretrain`` caches raw text with
    every token trained on, the chat tasks cache templated, masked rows, so a
    cache written for one must not load under the other.
    """
    ...
    # A new DataConfig field is classified in PREPROCESS_KEY_FIELDS or
    # NOT_PREPROCESS_KEY_FIELDS before it can merge (#1127). A new parameter here
    # is appended rather than inserted: the segment order is what
    # tests/preprocess_cache_blob.py reproduces for every schema generation.
    blob = (
        f"{_PREPROCESS_TOKENIZE_SCHEMA}\x1f{dataset_path}\x1f{tokenizer_name}"
        f"\x1f{max_length}\x1f{format_name}\x1f{chat_template or ''}"
        f"\x1f{mask_mode}\x1f{task}"
    )
