...
- more than 8 distinct layer shapes in one shard index → refused. Staging is allocated per shape and a shape with one member takes a full slot at any depth, so an index whose every layer differed would page-lock most of the model — which is what the disk tier exists to avoid. A real model has one to three
